...
function kernel_compute_force_p3!(force, sigmas, J, N::Int, R::Int)
    ...
    if check
        ...
        for a in 1:N, b in 1:N
            tmp += J[i, a, b] * sigmas[a, r] * sigmas[b, r] # J_iab * σ_a * σ_b
        end
...
function update_sigmas_p2!(force, sigmas, J, mu, beta, dt, N::Int, R::Int, block2D, grid2D)
    ...
    @. sigmas += -dt * mu * sigmas + sqrt(2dt/beta)*randn() + dt * force 
...
function evolve_sigmas!(force, sigmas, J, mu, beta, p, time, dt, tmax, N::Int, R::Int, block2D, grid2D)
    if p == 2 
        while time < tmax
